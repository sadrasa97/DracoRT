...
Supports Phi, Phi-2, Phi-3, Phi-4 architectures.
Features: learned position embeddings (Phi-2), RoPE (Phi-3/4), GQA, SwiGLU.
"""
...
logger = logging.getLogger("draco.models.phi")
...
class PhiAttention(nn.Module):
    def __init__(self, config: ModelConfig, layer_idx: int):
        ...
        self.q_proj = nn.Linear(self.hidden_size, self.num_heads * self.head_dim, bias=self.qk_bias)
        self.k_proj = nn.Linear(self.hidden_size, self.num_kv_heads * self.head_dim, bias=self.qk_bias)
...
class PhiDecoderLayer(nn.Module):
    def __init__(self, config: ModelConfig, layer_idx: int):
        super().__init__()
        self.self_attn = PhiAttention(config, layer_idx)
        self.mlp = PhiMLP(config)
        self.input_layernorm = PhiRMSNorm(config.hidden_size)
        self.post_attention_layernorm = PhiRMSNorm(config.hidden_size)
...
class PhiModel(nn.Module):
    def __init__(self, config: ModelConfig):
        ...
        self.layers = nn.ModuleList([PhiDecoderLayer(config, i) for i in range(config.num_hidden_layers)])
        self.norm = PhiRMSNorm(config.hidden_size)
...
class PhiAdapter(ModelAdapter):
    """Model adapter for Phi-family architectures."""
    ...
    def supported_model_types(self) -> List[str]:
        return ["phi", "phi3", "phi4", "phi-3", "phi-4"]

    def build_model(self) -> nn.Module:
        return PhiModel(self.config)
    ...
    def rotary_embedding(self, position_ids: torch.Tensor, head_dim: int) -> Tuple[torch.Tensor, torch.Tensor]:
        emb = PhiRotaryEmbedding(dim=head_dim, max_position_embeddings=self.config.max_position_embeddings, base=self.config.rope_theta)
