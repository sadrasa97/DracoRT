...
Supports Mistral, Mistral-7B, and related architectures.
Features: sliding window attention, GQA, RoPE.
"""
...
logger = logging.getLogger("draco.models.mistral")
...
class MistralAttention(nn.Module):
    ...
    def forward(
        ...
        # Sliding window mask, expressed in absolute key/query positions so
        # it stays correct once queries are offset by a nonzero cache length.
        ...
        # Causal mask (covers cached + new keys, offset-aware)
...
class MistralAdapter(ModelAdapter):
    """Model adapter for Mistral-family architectures."""
    ...
    def build_model(self) -> nn.Module:
        return MistralModel(self.config)
    ...
    def rotary_embedding(self, position_ids: torch.Tensor, head_dim: int) -> Tuple[torch.Tensor, torch.Tensor]:
        emb = MistralRotaryEmbedding(dim=head_dim, max_position_embeddings=self.config.max_position_embeddings, base=self.config.rope_theta)
